...
export default function ThemeDispo() {
...
const [selectedOption, setSelectedOption] = useState(null); // تخزين اختيار Yes أو No
const handleConfirm = () => {
    if (selectedOption === 'yes') {
      // تنفيذ منطق تعيين المواضيع العشوائية هنا
      console.log("Random topics assigned to students.");
    } else {
      console.log("Action cancelled.");
    }
    setOpenAddDialog(false); // إغلاق النافذة
  };
...
const handleAddSave = () => {
  ...
  {['list des themes disponble'].map((item, index) => (
    <Box
      key={index}
...
>
  ...
  <DialogContent sx={{ textAlign: "center", display: "flex", flexDirection: "column", gap: 2 }}>
    <Typography variant="body1" sx={{ fontSize: "1rem", fontWeight: "bold" }}>
      This button will assign a random topic from the available topics to each student without a topic.
    </Typography>
  ...
  <DialogActions sx={{ justifyContent: "center" }}>
    <Button
      onClick={handleConfirm} // دالة التأكيد التي ستنفذ الإجراء
      color="primary"
      variant="contained"
      sx={{
        textTransform: "none",
